...
class shopee_seller():
    ...
    def grab_seller(self):
        ...
        if c == "y" or not c:
            ...
            b = 1
            while (True):
                url_seller = base_url + "api/v4/search/search_user?keyword=" + self.katakunci + "&limit=100&offset=" + str(
                    a) + "&page=search_user&with_search_cover=true"
        ...
        for i in progressbar(f_read):
            try:
                ...
                try:
                    ...
                    desc = ""
                t_aktif = datetime.datetime.fromtimestamp(data['data']['last_active_time']).strftime(
                    '%d-%m-%Y %H:%M:%S')
                ...
                continue
            time.sleep(0.2)
        f_header = ['userid', 'shopid', 'username', 'nama_toko', 'negara', 'lokasi_toko', 'is_shopee_verified',
                    'is_official_shop', 'rating_normal', 'rating_bad', 'rating_good', 'cancelation_rate',
                    'total_rating', 'total_produk', 'total_follower', 'kec_respon_%', 'wkt_respon_detik', 'alamat',
                    'terakhir_aktif', 'deskripsi']
...
class shopee_keyword():
    ...
    def grab_keyword(self):
        ...
        while (True):
            if not self.lokasikey:
                url_produk = base_url + "api/v4/search/search_items?by=sales&keyword=" + self.katakunci + "&limit=100&newest=" + str(
                    a) + "&order=desc&page_type=search&scenario=PAGE_GLOBAL_SEARCH&skip_autocorrect=1&version=2"
            else:
                url_produk = base_url + "api/v4/search/search_items?by=sales&keyword=" + self.katakunci + "&limit=100&locations=" + self.lokasikey + "&newest=" + str(
                    a) + "&order=desc&page_type=search&scenario=PAGE_GLOBAL_SEARCH&skip_autocorrect=1&version=2"
        ...
        for i in progressbar(f_read):
            try:
                ...
                link_produk2 = base_url + link_produk1 + "-i." + str(i['item_basic']['shopid']) + "." + str(
                    i['item_basic']['itemid'])
                time.sleep(0.2)
                ...
                continue
        f_header = ['shopid', 'username_toko', 'lokasi_toko', 'itemid', 'produk', 'stok', 'terjual', 'histori_terjual',
                    'brand', 'jml_like', 'harga', 'harga_min', 'harga_max', 'harga_min_sebelum_disc',
                    'harga_max_sebelum_disc', 'diskon', 'url_produk']
...
class shopee():
    ...
    def grab_produk(self):
        ...
        b = 1
        while (True):
            url_produk = base_url + "api/v4/shop/search_items?limit=100&offset=" + str(a) + "&shopid=" + str(
                self.idseller)
        ...
        f_header = ['shopid', 'username_toko', 'nama_toko', 'lokasi_toko', 'itemid', 'produk', 'stok', 'terjual',
                    'histori_terjual', 'brand', 'jml_like', 'harga', 'harga_min', 'harga_max', 'harga_min_sebelum_disc',
                    'harga_max_sebelum_disc', 'diskon', 'url_produk']
